...
- 修正 `App` 曾固定传入 `unknown` 刷新状态的问题：现在经注入的仓储读取接口取得当前会话记录，定位最后一个已完成的采集步骤和对应材料规则，再调用 `getNextAvailableAt`。因此持续与每日重置规则可显示已知状态，人工规则展示内容包说明；UI 未直接访问 Dexie。
- 新增两项 App 集成测试，分别覆盖人工说明和持续规则的已知状态；本轮 RED 时两项均显示“无法估算”，GREEN 后通过。
- 将隐藏文件 input 设为 `tabIndex={-1}`，使其不占据键盘焦点；“导入备份”具名按钮仍触发文件选择，并由可访问性测试覆盖。
- 修正后最终验证：Vitest 15 文件 63/63、Playwright 2/2、生产构建和 `git diff --check` 均通过；构建仍只输出 Vite 的包体积建议。
